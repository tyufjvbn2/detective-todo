...
# Allow customizing the slash command name so the app can support non-English
# commands. Defaults to "/search".
SLASH_COMMAND = os.environ.get("SLACK_COMMAND", "/search")
logging.basicConfig(level=logging.DEBUG)
logging.debug("Using slash command handler for %s", SLASH_COMMAND)
...
def search_jira(
    query: str, base_url: str, email: str, api_token: str
) -> List[Dict]:
    """Search Jira issues."""
...
def search_confluence(
    ...
    if response.ok:
        ...
        return results
    logging.debug(
        "Confluence search failed with %d: %s",
...
def handle_search(ack, respond, command):
    ...
    if conf_base and conf_email and conf_token:
        conf_results = search_confluence(
            query, conf_base, conf_email, conf_token
        )
